...
{{--@endphp--}}
@inject('service', 'App\Http\Controllers\UserController')
			  ...
			  <tr class="tr-shadow">
				...
				<td>
				  @if($order->order_status == 1)
					<span class="status--process">Requested</span>
				  @endif
				  @if($order->order_status == 2)
					<span class="status--route">On Route</span>
				  @endif
				  @if($order->order_status == 3)
					<span class="status--delivered">Delivered</span>
				  @endif
				  @if($order->order_status == 4)
					<span class="status--denied">Canceled</span>
				  @endif
				</td>
				<td>{{$order->qty}}</td>
				<td>
				  <div class="table-data-feature">
					<a href="{{route('user.order.view',$order->orderId)}}" class="item" data-toggle="tooltip" data-placement="top" title="Send">
